...
export default function PinSettings() {
    ...
    return (
      <form onSubmit={handleSubmit} className="space-y-6 max-w-lg">
        ...
        <div className="bg-blue-50 dark:bg-blue-900/10 border border-blue-100 dark:border-blue-900/20 rounded-xl p-4 mb-6">
          <h3 className="text-sm font-bold text-blue-800 dark:text-blue-400 mb-1 flex items-center gap-2">
            <Shield className="w-4 h-4" /> Transaction Security
          </h3>
          <p className="text-xs text-blue-700 dark:text-blue-500">
            Your 4-digit PIN is required for withdrawals and sensitive wallet
            actions.
          </p>
        </div>
        ...
        <div className="grid grid-cols-2 gap-4">
          <div>
            ...
            <input
              ...
              value={data.new_pin}
              onChange={(e) =>
                setData({ ...data, new_pin: e.target.value.replace(/\D/g, "") })
              }
              className="w-full px-4 py-2 bg-white dark:bg-slate-900 border border-gray-200 dark:border-slate-700 rounded-lg focus:ring-2 focus:ring-violet-500/20 focus:border-violet-500 outline-none transition-all text-center tracking-widest text-gray-900 dark:text-gray-100 placeholder-gray-400"
              placeholder="••••"
            />
          </div>
          <div>
