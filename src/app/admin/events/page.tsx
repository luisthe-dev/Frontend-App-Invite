    ...
    <div className="p-8 max-w-7xl mx-auto w-full">
      <div className="flex flex-col md:flex-row md:items-center justify-between gap-4 mb-8">
        <div>
           ...
           <p className="text-slate-500 text-sm mt-1">Monitor and moderate event listings</p>
        </div>
        ...
        <div className="flex items-center gap-3">
            <div className="relative">
                ...
                <input 
                    ...
                    className="pl-9 pr-4 py-2 border border-slate-200 rounded-lg text-sm focus:outline-none focus:ring-2 focus:ring-violet-500 bg-white placeholder-slate-400 text-slate-900 shadow-sm transition-all hover:border-slate-300"
                    value={search}
                    ...
                    <tr key={event.id} className="hover:bg-slate-50/80 transition-colors group">
                        <td className="px-6 py-4">
                            <div className="flex items-center gap-4">
                                <div className="w-12 h-12 rounded-lg bg-slate-100 overflow-hidden flex-shrink-0 border border-slate-200 shadow-sm relative group-hover:shadow-md transition-shadow">
                                   {event.image_url ? (
                                ...
                                <div className="min-w-0">
                                    <p className="font-semibold text-slate-900 text-sm truncate max-w-[200px] group-hover:text-violet-700 transition-colors">{event.title}</p>
                                    <p className="text-xs text-slate-500 font-medium">{event.category || 'Uncategorized'}</p>
                                </div>
                            </div>
                        </td>
                        <td className="px-6 py-4">
                            <p className="text-sm font-medium text-slate-900">{event.user?.name || 'Unknown'}</p>
                        ...
                        <td className="px-6 py-4">
                            <span className={`px-2.5 py-1 rounded-full text-xs font-bold tracking-wide border ${
                                ...
                                event.status === 'suspended' ? 'bg-red-50 text-red-600 border-red-100' : ''
                            }`}>
                                {event.status}
                            </span>
                        </td>
