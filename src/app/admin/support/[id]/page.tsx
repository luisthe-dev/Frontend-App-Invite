...
export default function AdminTicketDetailPage() {
    ...
    return (
      <div className="flex h-screen bg-slate-50 overflow-hidden">
        ...
        <div className="w-80 bg-white border-r border-slate-200 hidden lg:flex flex-col shrink-0 z-20">
              ...
              <span
                className={`inline-flex items-center rounded-md px-2 py-1 text-xs font-medium ring-1 ring-inset uppercase ${
                  ticket.priority === "urgent"
                    ? "text-red-600 bg-red-50 ring-red-600/10"
              ...
              {ticket.transaction && (
                <div>
                  ...
                  <div className="bg-slate-50 border border-slate-100 rounded-lg p-3">
                    ...
                    <div className="text-xs capitalize flex items-center gap-1 mt-2">
                      <div
                        className={`w-2 h-2 rounded-full ${ticket.transaction.status === "payment_successful" ? "bg-emerald-500" : "bg-slate-300"}`}
                      />
              ...
              {isActive ? (
                <>
                  ...
                  <button
                    ...
                    className="w-full flex items-center justify-center gap-2 px-4 py-2 bg-white border border-slate-200 hover:bg-slate-50 text-slate-600 rounded-lg text-sm font-medium transition-colors"
                  >
        ...
        <div className="flex-1 flex flex-col min-w-0 bg-white">
          {/* Mobile Header */}
          <div className="lg:hidden border-b border-slate-200 px-4 py-3 flex items-center justify-between bg-white shadow-sm z-10">
          ...
          <div className="flex-1 overflow-y-auto p-4 sm:p-8 space-y-8 bg-slate-50/30">
              ...
              return (
                ...
                >
                  ...
                  >
                    ...
                    <div
                      className={`text-[10px] text-slate-400 mt-1 px-11 ${isAdmin ? "text-right" : "text-left"}`}
                    >
            ...
            {ticket.status === "closed" && (
              <div className="flex justify-center py-4">
                <div className="bg-slate-100 border border-slate-200 text-slate-500 px-4 py-2 rounded-lg text-sm flex items-center gap-2">
          ...
          {ticket.status !== "closed" && (
            <div className="p-4 bg-white border-t border-slate-100">
              <form
                onSubmit={handleSend}
                className="max-w-4xl mx-auto relative flex items-end gap-2 p-2 bg-slate-50 border border-slate-200 rounded-2xl focus-within:ring-2 focus-within:ring-violet-500/20 focus-within:border-violet-500 transition-all shadow-sm"
              >
                <button
                  type="button"
                  className="p-3 text-slate-400 hover:text-slate-600 hover:bg-slate-200/50 rounded-xl transition-colors"
                >
                  <Paperclip className="w-5 h-5" />
                </button>
                <textarea
                  className="flex-1 bg-transparent border-none focus:ring-0 p-3 min-h-[48px] max-h-[120px] resize-none outline-none text-slate-700 text-sm"
                ...
                <button
                  ...
                  className="p-3 bg-violet-600 hover:bg-violet-700 text-white rounded-xl disabled:opacity-50 disabled:cursor-not-allowed transition-all shadow-sm active:scale-95"
                >
                  {sending ? (
