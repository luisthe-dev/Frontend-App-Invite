...
export default function EventDetailsPage() {
  ...
  const faqs = [
    { q: "Is there a dress code for the event?", a: "Unless specified in the event description, the dress code is usually smart casual. Please check the 'About' section for specific details." },
    { q: "Are meals included with the ticket?", a: "This depends on the ticket type. VIP tickets often include meal vouchers. Standard tickets may not." },
    { q: "Will presentations be available after the event?", a: "Yes, presentation slides and recordings will be emailed to all attendees within 48 hours after the event concludes." }
  ];
  ...
  const paymentUrl = `/payment?event=${slug}&tickets=${encodeURIComponent(JSON.stringify(ticketCounts))}`;
  ...
  if (error || !event) {
      return (
          <div className="min-h-screen flex flex-col items-center justify-center bg-gray-50">
              ...
              <Link href="/events" className="px-6 py-2 bg-violet-600 text-white rounded-lg hover:bg-violet-700 transition-colors">
                  Browse Events
              </Link>
          </div>
      );
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 -mt-20 relative z-20">
            ...
            <div className="flex flex-col md:flex-row justify-between items-start gap-6">
                <div className="flex-1">
                    <div className="flex flex-wrap gap-2 mb-3 text-xs font-medium uppercase tracking-wider text-violet-600">
                        <span className="bg-violet-50 px-3 py-1 rounded-full">{event.category}</span>
                        {event.status === 'published' && <span className="bg-green-50 text-green-700 px-3 py-1 rounded-full">Live</span>}
                    </div>
        ...
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
                           ...
                           <div key={i} className="border border-gray-100 rounded-xl overflow-hidden">
                               <div 
                                ...
                                >
                                    ...
                                    <ChevronDown className={`w-4 h-4 text-gray-400 group-hover:text-gray-600 transition-transform ${openFAQ === i ? 'rotate-180' : ''}`} />
                               </div>
                               {openFAQ === i && (
            ...
            <div className="space-y-6">
                ...
                <div className="bg-white rounded-2xl shadow-sm border border-gray-100 p-6">
                    ...
                    <div className="mb-4">
                        <p className="font-semibold text-gray-900">{new Date(event.start_date).toLocaleDateString()}</p>
                        <p className="text-sm text-gray-500">{new Date(event.start_date).toLocaleTimeString()} - {new Date(event.end_date).toLocaleDateString()}</p>
                    </div>
                ...
                <div className="bg-white rounded-2xl shadow-sm border border-gray-100 p-6">
                            ...
                            event.tickets.map((ticket: any) => (
                                <div key={ticket.id}>
                                    ...
                                    <div className="flex items-center justify-between bg-gray-50 rounded-lg p-1">
                                        ...
                                        >-</button>
                                        <span className="font-medium text-sm w-8 text-center">{ticketCounts[ticket.id] || 0}</span>
                                        <button 
                                            onClick={() => updateCount(ticket.id, 1)}
                                            className="w-8 h-8 flex items-center justify-center bg-white rounded shadow-sm text-gray-600 hover:text-gray-900"
                                        >+</button>
                    ...
                    <div className="mt-8 pt-6 border-t border-gray-100">
                        ...
                        <Link href={paymentUrl} className={`block w-full py-3.5 bg-violet-600 hover:bg-violet-700 text-white font-bold rounded-xl text-center shadow-lg shadow-violet-200 transition-all active:scale-[0.98] ${total === 0 ? 'opacity-50 pointer-events-none' : ''}`}>
                            Purchase Tickets
                        </Link>
                    </div>
        ...
        <div className="fixed bottom-0 left-0 right-0 bg-white border-t border-gray-200 p-4 lg:hidden z-50">
            <div className="flex items-center justify-between max-w-7xl mx-auto">
                 ...
                 <Link href={paymentUrl} className={`px-8 py-3 bg-violet-600 text-white font-bold rounded-xl shadow-lg shadow-violet-200 ${total === 0 ? 'opacity-50 pointer-events-none' : ''}`}>
                    Purchase Tickets
                </Link>
            </div>
        </div>
